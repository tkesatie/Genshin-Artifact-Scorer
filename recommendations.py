...
- **generate_combos(slot_pools, current_artifacts)**: Yields all artifact set combinations from the candidate pools (for multi‑slot upgrades).
- **find_multi_piece_upgrades(char_name, char_config, current_artifacts, top_k_pools, current_stats, current_damage, roll_values, er_floor, damage_model, team_context, top_n=10)**: Ranks multi‑slot upgrade pathways by damage gain.
"""
...
# Import the new modules needed for multi‑piece calculations
from models import Artifact, BuildContext, CharacterStats
from stats_calculator import calculate_build_stats
from damage_calculator import calculate_damage_score
from set_bonus import apply_set_effects
...
# =============================================================================
# NEW MULTI‑PIECE UPGRADE FUNCTIONS
# =============================================================================

def generate_combos(
    slot_pools: Dict[str, List[Artifact]],
    current_artifacts: Dict[str, Artifact]
) -> Iterator[Dict[str, Artifact]]:
    """
    Yields all possible artifact set combinations from the candidate pools,
    ranging from swapping 1 slot up to all 5 slots.

    slot_pools: dict mapping slot name -> list of candidate Artifact dicts
    current_artifacts: dict mapping slot -> currently equipped Artifact (must have all 5 slots)

    Yields a dict mapping slot -> Artifact for each unique combination.
    Skips combinations where all chosen artifacts are identical to the currently
    equipped pieces for the selected slots (i.e., no actual change).
    """
    slots = list(slot_pools.keys())  # e.g., ['flower', 'feather', 'sands', 'goblet', 'circlet']

    # Ensure we have all 5 slots
    if len(slots) != 5:
        raise ValueError("slot_pools must contain exactly 5 slots.")

    # Precompute the list of candidates for each slot (including the current piece)
    # We'll use the current artifact as the "keep" choice.

    # Iterate over swap_size from 1 to 5
    for swap_size in range(1, 6):
        # Generate all combinations of slots to swap
        for selected_slots in itertools.combinations(slots, swap_size):
            # For the selected slots, we want to pick one candidate from each slot's pool
            # For unselected slots, we keep the current artifact.
            # We'll build a list of lists: for each selected slot, its candidate list.
            candidate_lists = [slot_pools[slot] for slot in selected_slots]

            # Product over these lists
            for chosen_artifacts in itertools.product(*candidate_lists):
                # Build the full set
                new_set = {}
                all_same = True
                for slot, chosen in zip(selected_slots, chosen_artifacts):
                    new_set[slot] = chosen
                    # Check if chosen is the same object as current
                    if chosen is not current_artifacts[slot]:
                        all_same = False
                # Fill unselected slots with current
                for slot in slots:
                    if slot not in new_set:
                        new_set[slot] = current_artifacts[slot]

                # Skip if all selected slots kept the same artifact (no actual change)
                if all_same:
                    continue
...
def find_multi_piece_upgrades(
    char_name: str,
    char_config: dict,
    current_artifacts: Dict[str, Artifact],
    top_k_pools: Dict[str, List[Artifact]],
    ...
    """
    Evaluates multi‑slot artifact swaps and returns a ranked list of upgrade pathways.

    Parameters:
        char_name: (unused, but kept for consistency)
        char_config: character configuration (used in BuildContext)
        current_artifacts: dict mapping slot -> currently equipped Artifact
        top_k_pools: dict mapping slot -> list of candidate Artifact objects (from Step 5)
        current_stats: CharacterStats of the current build
        current_damage: current total damage (raw * multipliers) for the character
        roll_values: roll value tables for stat calculation (passed to context)
        er_floor: minimum ER requirement (used by apply_er_gate)
        damage_model: which damage formula to use (e.g., "burst")
        team_context: external buffs (used in stats calculation)
        top_n: number of top combos to return

    Returns:
        List of dicts, each containing:
            - "new_set": dict mapping slot -> Artifact
            - "gain": relative gain (new_damage / current_damage - 1)
            - "stats": CharacterStats of the new build
            - "mods": set bonus modifiers (dict with at least "burst_multiplier")
        Sorted by gain descending.
    """
    results = []

    # Iterate over all combinations from the pools
    for new_set in generate_combos(top_k_pools, current_artifacts):
        # Build the context
        ...
        # Calculate stats
        new_stats = calculate_build_stats(context)

        # Apply ER gate
        er_penalty = apply_er_gate(new_stats, er_floor)
        ...
        # Assuming mods contains "burst_multiplier" (or default to 1.0)
        burst_mult = mods.get("burst_multiplier", 1.0)
        new_total_damage = raw_damage * burst_mult * er_penalty

        # Compute relative gain
        gain = (new_total_damage / current_damage) - 1.0
